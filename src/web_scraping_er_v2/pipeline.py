...
from web_scraping_er_v2.maps import (
    get_cc_map_country_list,
    generate_cc_map_df,
    read_sub_map,
)
from src.web_scraping_er_v2.load_to_bq import bq_load_daily, bq_load_map
from src.discord_integration.send_message import (
    send_simple_text,
    send_dataframe_as_text,
)
from src.common.utils import update_last_run
...
def pipeline(base_currency):
    ...
    exchange_rates_df = pd.DataFrame(
        data=exchange_rate_map.values(), index=pd.Index(exchange_rate_map.keys())
    )
    ...
    if result["status"] == "FAILED":
        ...
        error = result["error"]
        send_simple_text(f"BQ Daily run Failed,  error: {error}")
    else:
        # checking and sending subscribed exchange-rates
        sub_list = read_sub_map()
        logger.info("Sending ex-rates", subscribed=sub_list)
        discord_df = exchange_rates_df[
            exchange_rates_df["foreign_currency"].isin(sub_list)
        ]
